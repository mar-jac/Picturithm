...
class SortBox {
    ...
    static ready(lengthx, lengthy, divsx, divsy, duration, type) {
        console.log("SortBox.ready")
        // this.name = name;
        lengthx = Math.floor(lengthx / divsx) * divsx;
        lengthy = Math.floor(lengthy / divsy) * divsy;

        let img = ImageHandler.getImage()
        let scale = Math.min((lengthx / img.width), (lengthy / img.height));
        //lengthx = Math.floor(img.width / divsx) * divsx;
        // lengthy = Math.floor(img.height / divsy) * divsy;
        ...
        console.log(lengthy);
        console.log(scale);
        console.log(blocklenx);
        console.log(blockleny);
        this.canvas = [];
        for (let i = 0; i < divsx * divsy; ++i) {
            let can = document.createElement('canvas');
            can.height = blockleny;
            can.width = blocklenx;
            // console.log("SortBox canvas: " + blocklenx + ", " + blockleny);
            this.canvas.push(can);
        }
    }

    static readyImage(img) {
        console.log("SortBox.readyImage")
        // this.name = name;
        ...
        let imblocklenx = Math.floor(imlengthx / divsx);
        let imblockleny = Math.floor(imlengthy / divsy);
        let scale = Math.min((this.lengthx / img.width), (this.lengthy / img.height));

        $("#specific-sort0").css({width: SortBox.lengthx, height: SortBox.lengthy});
        for (let i = 0; i < divsy; ++i) {
            for (let j = 0; j < divsx; ++j) {
                let can = this.canvas[divsx * i + j]
                console.log("SX: " + (j * imblocklenx) + ", SY: " + (i * imblockleny) + ", width: " + imblocklenx + ", height: " + imblockleny + ", canvasX: 0, canvasY: 0, CanvasWidth: " + Math.floor(SortBox.lengthx / divsx) + ", CanvasHeight: " + Math.floor(SortBox.lengthy / divsy));
                // console.log("SortBox canvas3: " + (can.width) + ", " + (can.height));
                can.getContext("2d").drawImage(img, j * imblocklenx, i * imblockleny, imblocklenx, imblockleny, 0, 0, Math.floor(SortBox.lengthx / divsx), Math.floor(SortBox.lengthy / divsy));
            }
        }
    }

    constructor(name) {
        console.log("SortBox.constructor")
        this.name = name;
        this.imidarray = [];
        this.idarray = [];
        this.animcounter = 0;
        ...
        lengthx = Math.floor(img.width / divsx) * divsx;
        lengthy = Math.floor(img.height / divsy) * divsy;
        let blocklenx = Math.floor(lengthx / divsx);
        let blockleny = Math.floor(lengthy / divsy);

        let blocklenx_scaled = Math.floor(blocklenx * scale);
        let blockleny_scaled = Math.floor(blockleny * scale);

        container.css({"height": SortBox.lengthy + "px", "width": SortBox.lengthx + "px"});
        ...
        for (let i = 0; i < divsy; ++i) {
            for (let j = 0; j < divsx; ++j) {
                ...
                can.attr({
                    ...
                    "height": SortBox.lengthy / divsy,
                    "width": SortBox.lengthx / divsx
                });

                // console.log("SortBox canvas2: " + (SortBox.lengthx / divsx) + ", " + (SortBox.lengthy / divsy));
                let left = j * (SortBox.lengthx / divsx);
                let top = i * (SortBox.lengthy / divsy);
                can.css({
                    "top": top + "px",
                    "left": left + "px"
                });
    ...
    placeImage(id, imid) {
        ...
        let top = i * blockleny_scaled;
        let left = j * blocklenx_scaled;
        ob.css({
            "top": top + "px",
            "left": left + "px"
        });
...
class AppHandler {
    ...
    static ready(typeofSort) {
        console.log("AppHandler.ready")
        this.type = typeofSort;
        let img = ImageHandler.getImage()
        this.lengthy = Math.floor((img.height / img.width) * this.lengthx);
    ...
    static readySpecific(sortType, typeofSort) {
        console.log("AppHandler.readySpecific")
        this.type = typeofSort;
        this.specificSortType = sortType;
        let img = ImageHandler.getImage()
        this.lengthy = Math.floor((img.height / img.width) * this.lengthx);
        // Loading, Creating and Initializing the SortBox class.
        console.log("lengthx: " + this.lengthx + ", lengthy: " + this.lengthy);
        console.log("divsx: " + this.divsx + ", divsy: " + this.divsy);
        console.log("r1");
        SortBox.ready(this.lengthx, this.lengthy, this.divsx, this.divsy, this.duration, this.type); // this.type defines specific sort or all the sortings at once.
    ...
    static animationFinished() {
        ...
        if (arrayLength == 0 || arrayLength == "") {
            ...
            let scale = Math.min((SortBox.lengthx / this.img.width), (SortBox.lengthy / this.img.height));
            let divsx = $('#sortTilesAllx').val();
            let divsy = $('#sortTilesAlly').val();
            let w = Math.floor(SortBox.lengthx / divsx) * divsx;
            let h = Math.floor(SortBox.lengthy / divsy) * divsy;
            ...
            for (i = 0; i < arrayLength; i++) {
                if (this.all_sort_button_pressed_array[i] == 0) {
                    ...
                    $('#sort0').empty();
                    $('#sort0').html('<img id="animated-gif0" src="' + this.img.src + '" style="width: ' + w + 'px; height: ' + h + 'px; object-fit: cover;" />');
                    ...
                    $('#sort1').empty();
                    $('#sort1').html('<img id="animated-gif1" src="' + this.img.src + '" style="width: ' + w + 'px; height: ' + h + 'px; object-fit: cover;" />');
                    ...
                    $('#sort2').empty();
                    $('#sort2').html('<img id="animated-gif2" src="' + this.img.src + '" style="width: ' + w + 'px; height: ' + h + 'px; object-fit: cover;" />');
                    ...
                    $('#sort3').empty();
                    $('#sort3').html('<img id="animated-gif3" src="' + this.img.src + '" style="width: ' + w + 'px; height: ' + h + 'px; object-fit: cover;" />');
                    ...
                    $('#sort4').empty();
                    $('#sort4').html('<img id="animated-gif4" src="' + this.img.src + '" style="width: ' + w + 'px; height: ' + h + 'px; object-fit: cover;" />');
                    ...
                    $('#sort5').empty();
                    $('#sort5').html('<img id="animated-gif5" src="' + this.img.src + '" style="width: ' + w + 'px; height: ' + h + 'px; object-fit: cover;" />');
                } else {
    ...
    static animationFinishedForAllSort(sortType) {
        ...
        if (sortType == 0) {
            $('#sort0').empty();
            $('#sort0').html('<img id="animated-gif0" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        } else if (sortType == 1) {
            $('#sort1').empty();
            $('#sort1').html('<img id="animated-gif1" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        } else if (sortType == 2) {
            $('#sort2').empty();
            $('#sort2').html('<img id="animated-gif2" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        } else if (sortType == 3) {
            $('#sort3').empty();
            $('#sort3').html('<img id="animated-gif3" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        } else if (sortType == 4) {
            $('#sort4').empty();
            $('#sort4').html('<img id="animated-gif4" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        } else if (sortType == 5) {
            $('#sort5').empty();
            $('#sort5').html('<img id="animated-gif5" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        }
    ...
    static animationFinishedSpecific() {
        console.log("Animation animationFinishedSpecific");
        let scale = Math.min((SortBox.lengthx / this.img.width), (SortBox.lengthy / this.img.height));
        if (this.specificSortType == 0) {
            ...
            $('#specific-sort0').empty();
            $('#specific-sort0').html('<img id="animated-gif0" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        } else if (this.specificSortType == 1) {
            ...
            $('#specific-sort1').empty();
            $('#specific-sort1').html('<img id="animated-gif1" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        } else if (this.specificSortType == 2) {
            ...
            $('#specific-sort2').empty();
            $('#specific-sort2').html('<img id="animated-gif2" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
            ...
            $('#specific-sort3').empty();
            $('#specific-sort3').html('<img id="animated-gif3" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        } else if (this.specificSortType == 4) {
            ...
            $('#specific-sort4').empty();
            $('#specific-sort4').html('<img id="animated-gif4" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        } else if (this.specificSortType == 5) {
            ...
            $('#specific-sort5').empty();
            $('#specific-sort5').html('<img id="animated-gif5" src="' + this.img.src + '" style="width: ' + this.img.width * scale + 'px; height: ' + this.img.height * scale + 'px; object-fit: cover;" />');
        }
    }
}
...
class frontendAllSortingScreen extends FrontendViewControl {
    ...
    static ready() {
        if (this.loadReadyStatus == 0) {
            ...
            $('#sortTilesAlly').change(function () {
                let y_str = $(this).val();
                let y = Number.parseFloat(y_str);
                if (y_str === '' || !Number.isInteger(y) || y < 3 || y > 50) {
                    if(y_str === '')
                    {
                        $('#sortTilesAlly').val(3);
                    }
                    else if (y > 50) {
                        $('#sortTilesAlly').val(50);
                    } else {
                        $('#sortTilesAlly').val(Math.max(3, Math.floor(y)));
                    }
                    alert("Rows must be an integer number between 3 and 50");
                }
            });

            $('#sortTilesAllx').change(function () {
                let x_str = $(this).val();
                let x = Number.parseFloat(x_str);
                if (x_str === '' || !Number.isInteger(x) || x < 3 || x > 50) {
                    if(x_str === '')
                    {
                        $('#sortTilesAllx').val(3);
                    }
                    else if (x > 50) {
                        $('#sortTilesAllx').val(50);
                    } else {
                        $('#sortTilesAllx').val(Math.max(3, Math.floor(x)));
                    }
                    alert("Columns must be an integer number between 3 and 50");
                }
            });

            $('#all-sorting-set-tiles').click(function () {

                let sortTilesx = $('#sortTilesAllx').val();
                let sortTilesy = $('#sortTilesAlly').val();

                this.sortTilesx = sortTilesx;
                this.sortTilesy = sortTilesy;

                AppHandler.setTiles(this.sortTilesx, this.sortTilesy);
                AppHandler.clearAllSorts();
                AppHandler.ready(1);
                AppHandler.create();
            ...
            $('#all-sorting-reset').click(function () {
                ...
                $('.all-sorting-sort-checkboxes').prop("checked", false);
                $('.all-sorting-sort-checkboxes').attr("disabled", "disabled");
                $("#sortTilesAlly").val(AppHandler.divsy);
                $("#sortTilesAllx").val(AppHandler.divsx);
            });
            ...
            $("#all-sorting-button-export").click(function () {
                ...
                FormHandler.set_tiles($('#sortTilesAllx').val(), $('#sortTilesAlly').val());
                $.ajax({
                    type: "POST",
                    url: "export_all.php",
                    processData: false,
...
class frontendBubbleSortScreen extends FrontendViewControl {
    ...
    static sortTiles0x = 3;
    static sortTiles0y = 3;
    ...
    static ready() {
        if (this.loadReadyStatus0 == 0) {
            ...
            $('#bubble-sort-set-tilesy').change(function () {
                let y_str = $(this).val();
                let y = Number.parseFloat(y_str);
                if (y_str === '' || !Number.isInteger(y) || y < 3 || y > 50) {
                    if(y_str === '')
                    {
                        $('#bubble-sort-set-tilesy').val(3);
                    }
                    else if (y > 50) {
                        $('#bubble-sort-set-tilesy').val(50);
                    } else {
                        $('#bubble-sort-set-tilesy').val(Math.max(3, Math.floor(y)));
                    }
                    alert("Rows must be an integer number between 3 and 50");
                }
            });

            $('#bubble-sort-set-tilesx').change(function () {
                let x_str = $(this).val();
                let x = Number.parseFloat(x_str);
                if (x_str === '' || !Number.isInteger(x) || x < 3 || x > 50) {
                    if(x_str === '')
                    {
                        $('#bubble-sort-set-tilesx').val(3);
                    }
                    else if (x > 50) {
                        $('#bubble-sort-set-tilesx').val(50);
                    } else {
                        $('#bubble-sort-set-tilesx').val(Math.max(3, Math.floor(x)));
                    }
                    alert("Columns must be an integer number between 3 and 50");
                }
            });
            ...
            $('#bubble-sort-button-set-tiles').click(function () {
                ...
                let sortTiles0x = $('#bubble-sort-set-tilesx').val();
                let sortTiles0y = $('#bubble-sort-set-tilesy').val();
                this.sortTiles0x = sortTiles0x;
                this.sortTiles0y = sortTiles0y;

                AppHandler.setTiles(sortTiles0x, sortTiles0y);
                AppHandler.clearAllSorts();
                AppHandler.readySpecific(0, 2);
                AppHandler.createSpecific();
            ...
            $("#bubble-sort-button-export").click(function () {
                frontendExport.show();
                frontendBubbleSortScreen.hide();
                FormHandler.set_tiles($('#bubble-sort-set-tilesx').val(), $('#bubble-sort-set-tilesy').val());
                $.ajax({
                    type: "POST",
                    url: "export_0.php",
                    processData: false,
...
class frontendSelectionSortScreen extends FrontendViewControl {
    ...
    static sortTiles1x = 3;
    static sortTiles1y = 3;
    ...
    static ready() {
        if (this.loadReadyStatus1 == 0) {
            ...
            $('#selection-sort-set-tilesy').change(function () {
                let y_str = $(this).val();
                let y = Number.parseFloat(y_str);
                if (y_str === '' || !Number.isInteger(y) || y < 3 || y > 50) {
                    if(y_str === '')
                    {
                        $('#selection-sort-set-tilesy').val(3);
                    }
                    else if (y > 50) {
                        $('#selection-sort-set-tilesy').val(50);
                    } else {
                        $('#selection-sort-set-tilesy').val(Math.max(3, Math.floor(y)));
                    }
                    alert("Rows must be an integer number between 3 and 50");
                }
            });

            $('#selection-sort-set-tilesx').change(function () {
                let x_str = $(this).val();
                let x = Number.parseFloat(x_str);
                if (x_str === '' || !Number.isInteger(x) || x < 3 || x > 50) {
                    if(x_str === '')
                    {
                        $('#selection-sort-set-tilesx').val(3);
                    }
                    else if (x > 50) {
                        $('#selection-sort-set-tilesx').val(50);
                    } else {
                        $('#selection-sort-set-tilesx').val(Math.max(3, Math.floor(x)));
                    }
                    alert("Columns must be an integer number between 3 and 50");
                }
            });
            ...
            $('#selection-sort-button-set-tiles').click(function () {
                ...
                let sortTiles1x = $('#selection-sort-set-tilesx').val();
                let sortTiles1y = $('#selection-sort-set-tilesy').val();
                this.sortTiles1x = sortTiles1x;
                this.sortTiles1y = sortTiles1y;

                AppHandler.setTiles(sortTiles1x, sortTiles1y);
                AppHandler.clearAllSorts();
                AppHandler.readySpecific(1, 2);
                AppHandler.createSpecific();
            ...
            $("#selection-sort-button-export").click(function () {
                frontendExport.show();
                frontendSelectionSortScreen.hide();
                FormHandler.set_tiles($('#selection-sort-set-tilesx').val(), $('#selection-sort-set-tilesy').val());
                $.ajax({
                    type: "POST",
                    url: "export_1.php",
                    processData: false,
...
class frontendInsertionSortScreen extends FrontendViewControl {
    ...
    static sortTiles2x = 3;
    static sortTiles2y = 3;
    ...
    static ready() {
        if (this.loadReadyStatus2 == 0) {
            ...
            $('#insertion-sort-set-tilesy').change(function () {
                let y_str = $(this).val();
                let y = Number.parseFloat(y_str);
                if (y_str === '' || !Number.isInteger(y) || y < 3 || y > 50) {
                    if(y_str === '')
                    {
                        $('#insertion-sort-set-tilesy').val(3);
                    }
                    else if (y > 50) {
                        $('#insertion-sort-set-tilesy').val(50);
                    } else {
                        $('#insertion-sort-set-tilesy').val(Math.max(3, Math.floor(y)));
                    }
                    alert("Rows must be an integer number between 3 and 50");
                }
            });

            $('#insertion-sort-set-tilesx').change(function () {
                let x_str = $(this).val();
                let x = Number.parseFloat(x_str);
                if (x_str === '' || !Number.isInteger(x) || x < 3 || x > 50) {
                    if(x_str === '')
                    {
                        $('#insertion-sort-set-tilesx').val(3);
                    }
                    else if (x > 50) {
                        $('#insertion-sort-set-tilesx').val(50);
                    } else {
                        $('#insertion-sort-set-tilesx').val(Math.max(3, Math.floor(x)));
                    }
                    alert("Columns must be an integer number between 3 and 50");
                }
            });
            ...
            $('#insertion-sort-button-set-tiles').click(function () {
                ...
                let sortTiles2x = $('#insertion-sort-set-tilesx').val();
                let sortTiles2y = $('#insertion-sort-set-tilesy').val();
                this.sortTiles2x = sortTiles2x;
                this.sortTiles2y = sortTiles2y;

                AppHandler.setTiles(sortTiles2x, sortTiles2y);
                AppHandler.clearAllSorts();
                AppHandler.readySpecific(2, 2);
                AppHandler.createSpecific();
            ...
            $("#insertion-sort-button-export").click(function () {
                frontendExport.show();
                frontendInsertionSortScreen.hide();
                FormHandler.set_tiles($('#insertion-sort-set-tilesx').val(), $('#insertion-sort-set-tilesy').val());
                $.ajax({
                    type: "POST",
                    url: "export_2.php",
                    processData: false,
...
class frontendMergeSortScreen extends FrontendViewControl {
    ...
    static sortTiles3x = 3;
    static sortTiles3y = 3;
    ...
    static ready() {
        if (this.loadReadyStatus3 == 0) {
            ...
            $('#merge-sort-set-tilesy').change(function () {
                let y_str = $(this).val();
                let y = Number.parseFloat(y_str);
                if (y_str === '' || !Number.isInteger(y) || y < 3 || y > 50) {
                    if(y_str === '')
                    {
                        $('#merge-sort-set-tilesy').val(3);
                    }
                    else if (y > 50) {
                        $('#merge-sort-set-tilesy').val(50);
                    } else {
                        $('#merge-sort-set-tilesy').val(Math.max(3, Math.floor(y)));
                    }
                    alert("Rows must be an integer number between 3 and 50");
                }
            });

            $('#merge-sort-set-tilesx').change(function () {
                let x_str = $(this).val();
                let x = Number.parseFloat(x_str);
                if (x_str === '' || !Number.isInteger(x) || x < 3 || x > 50) {
                    if(x_str === '')
                    {
                        $('#merge-sort-set-tilesx').val(3);
                    }
                    else if (x > 50) {
                        $('#merge-sort-set-tilesx').val(50);
                    } else {
                        $('#merge-sort-set-tilesx').val(Math.max(3, Math.floor(x)));
                    }
                    alert("Columns must be an integer number between 3 and 50");
                }
            });
            ...
            $('#merge-sort-button-set-tiles').click(function () {
                ...
                let sortTiles3x = $('#merge-sort-set-tilesx').val();
                let sortTiles3y = $('#merge-sort-set-tilesy').val();
                this.sortTiles3x = sortTiles3x;
                this.sortTiles3y = sortTiles3y;

                AppHandler.setTiles(sortTiles3x, sortTiles3y);
                AppHandler.clearAllSorts();
                AppHandler.readySpecific(3, 2);
                AppHandler.createSpecific();
            ...
            $("#merge-sort-button-export").click(function () {
                frontendExport.show();
                frontendMergeSortScreen.hide();
                FormHandler.set_tiles($('#merge-sort-set-tilesx').val(), $('#merge-sort-set-tilesy').val());
                $.ajax({
                    type: "POST",
                    url: "export_3.php",
                    processData: false,
...
class frontendShellSortScreen extends FrontendViewControl {
    ...
    static sortTiles4x = 3;
    static sortTiles4y = 3;
    ...
    static ready() {
        if (this.loadReadyStatus4 == 0) {
            ...
            $('#shell-sort-set-tilesy').change(function () {
                let y_str = $(this).val();
                let y = Number.parseFloat(y_str);
                if (y_str === '' || !Number.isInteger(y) || y < 3 || y > 50) {
                    if(y_str === '')
                    {
                        $('#shell-sort-set-tilesy').val(3);
                    }
                    else if (y > 50) {
                        $('#shell-sort-set-tilesy').val(50);
                    } else {
                        $('#shell-sort-set-tilesy').val(Math.max(3, Math.floor(y)));
                    }
                    alert("Rows must be an integer number between 3 and 50");
                }
            });

            $('#shell-sort-set-tilesx').change(function () {
                let x_str = $(this).val();
                let x = Number.parseFloat(x_str);
                if (x_str === '' || !Number.isInteger(x) || x < 3 || x > 50) {
                    if(x_str === '')
                    {
                        $('#shell-sort-set-tilesx').val(3);
                    }
                    else if (x > 50) {
                        $('#shell-sort-set-tilesx').val(50);
                    } else {
                        $('#shell-sort-set-tilesx').val(Math.max(3, Math.floor(x)));
                    }
                    alert("Columns must be an integer number between 3 and 50");
                }
            });
            ...
            $('#shell-sort-button-set-tiles').click(function () {
                ...
                let sortTiles4x = $('#shell-sort-set-tilesx').val();
                let sortTiles4y = $('#shell-sort-set-tilesy').val();
                this.sortTiles4x = sortTiles4x;
                this.sortTiles4y = sortTiles4y;

                AppHandler.setTiles(sortTiles4x, sortTiles4y);
                AppHandler.clearAllSorts();
                AppHandler.readySpecific(4, 2);
                AppHandler.createSpecific();
            ...
            $("#shell-sort-button-export").click(function () {
                frontendExport.show();
                frontendShellSortScreen.hide();
                FormHandler.set_tiles($('#shell-sort-set-tilesx').val(), $('#shell-sort-set-tilesy').val());
                $.ajax({
                    type: "POST",
                    url: "export_4.php",
                    processData: false,
...
class frontendQuickSortScreen extends FrontendViewControl {
    ...
    static sortTiles5x = 3;
    static sortTiles5y = 3;
    ...
    static ready() {
        if (this.loadReadyStatus5 == 0) {
            ...
            $('#quick-sort-set-tilesy').change(function () {
                let y_str = $(this).val();
                let y = Number.parseFloat(y_str);
                if (y_str === '' || !Number.isInteger(y) || y < 3 || y > 50) {
                    if(y_str === '')
                    {
                        $('#quick-sort-set-tilesy').val(3);
                    }
                    else if (y > 50) {
                        $('#quick-sort-set-tilesy').val(50);
                    } else {
                        $('#quick-sort-set-tilesy').val(Math.max(3, Math.floor(y)));
                    }
                    alert("Rows must be an integer number between 3 and 50");
                }
            });

            $('#quick-sort-set-tilesx').change(function () {
                let x_str = $(this).val();
                let x = Number.parseFloat(x_str);
                if (x_str === '' || !Number.isInteger(x) || x < 3 || x > 50) {
                    if(x_str === '')
                    {
                        $('#quick-sort-set-tilesx').val(3);
                    }
                    else if (x > 50) {
                        $('#quick-sort-set-tilesx').val(50);
                    } else {
                        $('#quick-sort-set-tilesx').val(Math.max(3, Math.floor(x)));
                    }
                    alert("Columns must be an integer number between 3 and 50");
                }
            });
            ...
            $('#quick-sort-button-set-tiles').click(function () {
                ...
                let sortTiles5x = $('#quick-sort-set-tilesx').val();
                let sortTiles5y = $('#quick-sort-set-tilesy').val();
                this.sortTiles5x = sortTiles5x;
                this.sortTiles5y = sortTiles5y;

                AppHandler.setTiles(sortTiles5x, sortTiles5y);
                AppHandler.clearAllSorts();
                AppHandler.readySpecific(5, 2);
                AppHandler.createSpecific();
            ...
            $("#quick-sort-button-export").click(function () {
                frontendExport.show();
                frontendQuickSortScreen.hide();
                FormHandler.set_tiles($('#quick-sort-set-tilesx').val(), $('#quick-sort-set-tilesy').val());
                $.ajax({
                    type: "POST",
                    url: "export_5.php",
                    processData: false,
...
class FormHandler {
    ...
    static set_selected_algorithms(algorithms) {
        this.formdata.append("algorithms", algorithms);
    }

    //algorithms comma separated indexes (0,5,4)
    static set_tiles(tilesx, tilesy) {
        this.formdata.append("tilesx", tilesx);
        this.formdata.append("tilesy", tilesy);
    }
