...
lengthx = size[0]
lengthy = size[1]
divsx = int(sys.argv[4])
divsy = int(sys.argv[5])
...
lineheight = 15
boxes = []
vidheight = 2 * lengthy + 2 * lineheight + 3 * padding
vidwidth = 3 * lengthx + 4 * padding
gif_frames = []
...
positions = [
    (padding + (_lengthx - 110) // 2, padding + _lengthy),
    (_lengthx + 2 * padding + (_lengthx - 140) // 2, padding + _lengthy),
    (2 * _lengthx + 3 * padding + (_lengthx - 140) // 2, padding + _lengthy),
    (padding + (_lengthx - 210) // 2, 2 * _lengthy + lineheight + 2 * padding),
    (_lengthx + 2 * padding + (_lengthx - 280) // 2, 2 * _lengthy + lineheight + 2 * padding),
    (2 * _lengthx + 3 * padding + (_lengthx - 100) // 2, 2 * _lengthy + lineheight + 2 * padding),
]
...
labels = [
    ...
    "Insertion Sort",
    "Merge Sort (in-place)",
    "Shell Sort (Knuth's formula)",
    "Quick Sort",
]
background = Image.new('RGB', (vidwidth, vidheight))

for algorithm_index in algorithms:
    boxes.append(SortBox())
    alg_index = int(algorithm_index)
    boxes[len(boxes) - 1].applyalgo(algos[alg_index])
    ImageDraw.Draw(background).text(positions[alg_index], labels[alg_index], (255, 255, 255), ImageFont.truetype("cour.ttf", 16))

background = cvutil.piltocv(background)
positions = [
    (padding, padding),
    (_lengthx + 2 * padding, padding),
    (2 * _lengthx + 3 * padding, padding),
    (padding, _lengthy + lineheight + 2 * padding),
    (_lengthx + 2 * padding, _lengthy + lineheight + 2 * padding),
    (2 * _lengthx + 3 * padding, _lengthy + lineheight + 2 * padding),
]
for i in range(len(boxes[0].anim.frames())):
    frame = numpy.copy(background)
    for j, algorithm_index in enumerate(algorithms):
        alg_index = int(algorithm_index)
        cvutil.paste(frame, positions[alg_index], boxes[j].anim.frames()[i], vidwidth, vidheight)
